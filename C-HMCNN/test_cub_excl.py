...
def main():
    ...
    if not args.no_constraints:
        ...
        if not os.path.isfile('constraints/' + dataset_name + '.sdd') or not os.path.isfile('constraints/' + dataset_name + '.vtree'):
            ...
            alpha = mgr.true() #becoming more and more restrictive
            alpha.ref()
            ...
            #initialize gamma for ME
            gamma = mgr.false() #gamma  (s[1] | s[2] | s[3] [200]) most restrictive condition = mgr.false
            gamma.ref()
            ...
            for i in range(R.size(0)): #for all genera g
                ...
                for idx1 in range(len(species)):
                    for idx2 in range(idx1 + 1, len(species)): # all species after s1
                        ...
                        old_gamma = gamma
                        gamma = gamma & (-mgr.vars[s1+1] | -mgr.vars[s2+1]) #one clause must be true # sdd count starts at 1
                        gamma.ref()
                        old_gamma.deref()
    ...
    checkpoint_model = torch.load("/mnt/cimec-storage6/users/nguyenanhthu.tran/intern25/spl/SPL/C-HMCNN/models/model_b_with_constraints/cub_others_ConstrainedFFNNModel_20250405-000154_256_0.001_avgloss_model.pth")
    checkpoint_gate = torch.load("/mnt/cimec-storage6/users/nguyenanhthu.tran/intern25/spl/SPL/C-HMCNN/models/model_b_with_constraints/cub_others_ConstrainedFFNNModel_20250405-000154_256_0.001_avgloss_gate.pth")
        ...
        for epoch in range(num_epochs):
            ...
            # Early stopping    
                ...
                if patience == 0:
                    ...
                    #For the difference file, please rerun the best-performing model (saved after finetuning) with --no-train
